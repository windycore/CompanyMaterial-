...
#define  SWITCH_CMD_COTINUE    // 切花指令可以1切多
...
uint8_t OutHDCP_Flag[MAX_OUT]={0};
uint16_t OutHDCP_TimeDelay[MAX_OUT]={0};

static clock_time_t EdidUpataStart = 0;
...
#if ((MACHINETYPE == MUH4K862)||(MACHINETYPE==MHUB4K431))
void SetUartDelayReceiveForPoweronFunction(unsigned int u16Time)
{
    SiiPlatformTimerSet(etPWON_DELAY,mS_TO_TICKS(u16Time),0xFF);
}

unsigned char GetUartReceiveStar(void)
{
    if(SiiPlatformTimerGet(etPWON_DELAY)!=0)
    {
...
void UART1_SendBytes(const unsigned char  *Data, u8 num)
{
    u8 i = 0;
    ...
    if ((USART_EDIDdata_count == 128) || (USART_EDIDdata_count == 256))
    {
        ...
        if (crc == 0)
        {
            ...
            else if (EDID_updata_port == edsEDID_INTERN1)
            {
                for (j = 0; j < MAX_IN; j++)
                {
                    appSystemInputPortEdidDataSet(j, (eEDIDSELECT)(edsEDID_IN1 + j), USART_EDIDdata);
                    IWDG_ReloadCounter(); //喂狗
                }
                UART1_SendBytes("EDID Upgrade OK!\r\n", strlen("EDID Upgrade OK!\r\n"));
            }
...
{        Event_t   bEvent;
      //  appUartSwitchInfoSet(cInPort, i,pOutPortList,TRUE);
      //  appSystemSwitcherPortSet(cInPort, i, pOutPortList);  // 0V
      		//Event_t   bEvent;
...
void HexProessVedioIRSwitch(BYTE cInPort, unsigned int i, PBYTE acOutPortList)
{    
            Event_t   bEvent;
...
void ProcessOneOpen(unsigned char OutPut, BOOL Re_Message)
{
	...
	if ((psOutputPortMgrData->acInPortSelect[OutPut - 1]) >= MAX_IN)
	{
	    ...
	    appSystemSwitcherSet2(psOutputPortMgrData->acInPortSelect[OutPut - 1], 1, OutPut - 1);
        
        #ifdef PRD_IR_OUT
        appSystemIRSwitcherSet(psOutputPortMgrData->acIRInPortSelect[OutPut - 1],1,OutPut - 1);
        #endif
        #if ((MACHINETYPE == MUH4K862)||(MACHINETYPE==MHUB4K431))
...
void ProCopyEDIDDataOutport2Inport(unsigned char HPnum1, unsigned char HPnum2)
{
    ...
    if (HPnum2 == MAX_IN+1)
    {
        for (i = 0; i < MAX_IN; i++)
        {
            appSystemInputPortEdidSet(i, (eEDIDSELECT)(edsEDID_EXTERN1 + HPnum1 - 1));

        }
    }
    else
    {
        appSystemInputPortEdidSet(HPnum2 - 1, (eEDIDSELECT)(edsEDID_EXTERN1 + HPnum1 - 1));
    }
...
void  ProQueryVedioSwitchHexStatus(unsigned char cOutput)
{  BYTE acMessage2[USART1_TX_BUFF_MAX] = {0xA5,0x5B,0x02,0x01, 0x02 ,0x00 ,0x01, 0x00 ,0x00 ,0x00 ,0x00 ,0x00};
       POUTPUTPORT_MANAGER_DATA    psOutputPortMgrData;
   
    psOutputPortMgrData  = (POUTPUTPORT_MANAGER_DATA)utilDataMgrWriteLockData(nnOUTPUTPORT_MANAGER_DATA);
         acMessage2[4]=cOutput+1;   
         acMessage2[6]=psOutputPortMgrData->acInPortSelect[cOutput]+1;   
      utilDataMgrUnlock(nnOUTPUTPORT_MANAGER_DATA,
                      FALSE);
...
void ProRecallFactoryDefault()
{
    ...
    for (i = 0; i < MAX_IN; i++)
    {
        appSystemInputPortEdidSet(i, (eEDIDSELECT)(edsEDID_IN1 + i));
    }
    for (j = 0; j < MAX_OUT; j++)
    {
        //digit_audio[j] = 1;
...
void Function_SetRemoteCtrlMcu(BOOL bEn)
{
    PSYSTEM_SETTING_DATA   psSystemSettingData;
    psSystemSettingData = (PSYSTEM_SETTING_DATA)utilDataMgrWriteLockData(nnSYSTEM_SETTING_DATA);
    if(bEn == TRUE)
    {
        psSystemSettingData->acRemoteCtrlMcuFlag = TRUE;
        UART1_SendBytes("Remote control Mcu: Enable!\r\n", strlen("Remote control Mcu: Enable!\r\n"));
        dvCpld_RemoteCtrlMcuMode(psSystemSettingData->acRemoteCtrlMcuFlag); 
    }
    else
    {
        psSystemSettingData->acRemoteCtrlMcuFlag = FALSE;
        UART1_SendBytes("Remote control Mcu: Disable!\r\n", strlen("Remote control Mcu: Disable!\r\n"));
        dvCpld_RemoteCtrlMcuMode(psSystemSettingData->acRemoteCtrlMcuFlag); 
    }
    utilDataMgrUnlock(nnSYSTEM_SETTING_DATA, TRUE);
}

void Function_QueryRemoteCtrlMcuStatus(void)
{
    PSYSTEM_SETTING_DATA   psSystemSettingData;
    psSystemSettingData = (PSYSTEM_SETTING_DATA)utilDataMgrWriteLockData(nnSYSTEM_SETTING_DATA);
    if(psSystemSettingData->acRemoteCtrlMcuFlag == TRUE)
    {
        UART1_SendBytes("Remote control Mcu: Enable!\r\n", strlen("Remote control Mcu: Enable!\r\n"));
    }
    else
...
void Function_QueryRemoteBaudrateStatus(BYTE u8Remote)
{
    PSYSTEM_SETTING_DATA   psSystemSettingData;
    
    BYTE cCount = 0;
	BYTE acMessage2[50] = { 0 };
	memset(acMessage2, 0, 50);
	
    psSystemSettingData = (PSYSTEM_SETTING_DATA)utilDataMgrWriteLockData(nnSYSTEM_SETTING_DATA);
    
    #if ((MACHINETYPE == MUH4K862)||(MACHINETYPE == MHUB4K431))
    u8Remote = u8Remote;
    cCount += SiiSprintf((char*)&acMessage2[cCount],"Remote port baudrate is ");
    switch(u8RemoteBaudRate)
    #else
    cCount += SiiSprintf((char*)&acMessage2[cCount],"Remote port %d baudrate is ", u8Remote);
    switch(psSystemSettingData->acRemoteBaudRate[u8Remote-1])
    #endif
    {
        case 1:
            cCount += SiiSprintf((char*)&acMessage2[cCount],"%s!\r\n", "2400");
            break;
        case 2:
            cCount += SiiSprintf((char*)&acMessage2[cCount],"%s!\r\n", "4800");
            break;
    ...
    UART1_SendBytes(acMessage2, cCount);
  ...
  //  Event_t   bEvent;
   ...
   // PINPUTPORT_MANAGER_DATA       psInputPortMgrData;
    ...
    if(*USART1_rxtailpoint == 0xa5 && *(USART1_rxtailpoint+1)==0x5b)
    {
        ...
        {
            if(*(USART1_rxtailpoint+2)==0x02&&*(USART1_rxtailpoint+3)==0x03)
            {
                if(StrCmp(USART1_rxtailpoint, cmdlist[i], USART1_rxprocesspoint) != NULL)
                {
                    ...
                    HexProessVedioIRSwitch(cInPort, 1, acOutPortList);
                    return (CH_CORRECT);
                }
            }
            else if(*(USART1_rxtailpoint+2)==0x02&&*(USART1_rxtailpoint+3)==0x01)
            {
                     cOutPort = *(USART1_rxtailpoint+4)-1;

                    if((cOutPort>MAX_OUT))
                        return (CM_ERROR);

			 ProQueryVedioSwitchHexStatus(cOutPort);
...
#ifdef PRD_POWER_ONOFF_MESSAGE
            //#if (MAX_OUT>4)
            else if ((cRemote >= 'a') && (cRemote <= 'a'+MAX_OUT-1) || // a,b,c,d,e,f,g,h时表示设定在PWON时往对应的远端发送的数据；
                     (cRemote >= 'A') && (cRemote <= 'A'+MAX_OUT-1))  //  A,B,C,D,E,F,G,H  时表示设定在PWOFF时往对应的1、2、3、4路远端发送的数据?
            {
                BYTE cbPowrON;
...
//========================================================================================
        ...
        {
            HPnum1 = *pp - 0x30;
            if ((HPnum1 > 0) && (HPnum1 <= (edsEDID_EXTERN1-edsEDID_INTERN1)))
            {
...
//===========================================================
        ...
        */
        ...
        #endif
        if(StrCmp(USART1_rxtailpoint, "RemoteCtrl Open.",USART1_rxprocesspoint) != NULL)
        {
            Function_SetRemoteCtrlMcu(TRUE);
            return(CH_CORRECT);    
        }
        if(StrCmp(USART1_rxtailpoint, "RemoteCtrl Close.",USART1_rxprocesspoint) != NULL)
        {
            Function_SetRemoteCtrlMcu(FALSE);
            return(CH_CORRECT);    
        }
        if(StrCmp(USART1_rxtailpoint, "Get RemoteCtrl.",USART1_rxprocesspoint) != NULL)
        {
            Function_QueryRemoteCtrlMcuStatus();
            return(CH_CORRECT);    
        }
        #if ((MACHINETYPE == MUH4K862)||(MACHINETYPE == MHUB4K431))
        if(StrCmp(USART1_rxtailpoint, "Get RemoteBaudrate.",USART1_rxprocesspoint) != NULL)
        {
            Function_QueryRemoteBaudrateStatus(0);
            return(CH_CORRECT);    
        }
        #else
        if(StrCmp(USART1_rxtailpoint, "Get RemoteBaudrate",USART1_rxprocesspoint) != NULL)
        {
            pp = USART1_rxtailpoint+strlen("Get RemoteBaudrate");
            #if (MACHINETYPE == MUH4K862)
            if((*pp>'0')&&(*pp<=MAX_OUT-2+'0'))
            #elif (MACHINETYPE == MHUB4K431)
            if((*pp>'0')&&(*pp<=MAX_OUT-1+'0'))
            #else
            if((*pp>'0')&&(*pp<=MAX_OUT+'0'))
            #endif
            {
                cOutPort = *pp-'0';
                if(*(pp+1)!='.')
                    return (CM_ERROR);
                Function_QueryRemoteBaudrateStatus(cOutPort);
                return(CH_CORRECT);    
            }
            return (CM_ERROR);
        }
...
// 查询
	...
	if (StrCmp(USART1_rxtailpoint, "/^TestVersion;", USART1_rxprocesspoint) != NULL) // Software Version
        {
                 UART1_SendData2PC(TEST_FW_Version, strlen(TEST_FW_Version));
               return (OTHERS);
         }
#if 0
...
void Uart1_process(void)
{
  ...
  // 垃圾代码
    if (g_bUSART1_DataComplete == 1)
    {
        ...
        if (USART_data_flag == 0)
        {
            #if 0 // wf8421 for test
            for(i=0;i<USART1_rxpoint-USART1_RX_BUFF;i++)
            {
                APP_Printf("%c",USART1_RX_BUFF[i]);
            }
            APP_Printf("\n");
            i=0;
            #endif
...
//Huang 2016-Jun-28, 17:35:10
			    ...
			    )
                {
                  if((USART1_rxprocesspoint < USART1_rxpoint))
                    ...
                    // add by kyq 20150929                                                                        //USART_LCD(temp);
...
//串口HEX 数据发给DB9(PC)
void UART1_SendHexData2PC(PBYTE pcBuf, BYTE cLength)
{
