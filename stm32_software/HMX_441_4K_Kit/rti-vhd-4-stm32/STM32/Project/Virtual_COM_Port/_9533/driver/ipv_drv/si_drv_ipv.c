...
void SiiDrvIpvModeSet ( SiiIpvMode_t mode )
{
    ...
    switch ( mode )
    {
        ...
        case SiiIpvModeACTIVE:
            ...
            SiiRegWrite( cInstance,  REG_IPV_MODE, mode );
            ...
            //SiiRegWrite( cInstance,  REG_PAUTH_RPOVR, CLEAR_BITS );
        	//SiiRegWrite( cInstance,  REG_PAUTH_RP_AOVR, CLEAR_BITS );
        	//SiiRegModify(REG_RX_PORT_SEL, MSK_RP_PORT_SEL, CLEAR_BITS);

            break;
        case SiiIpvModeSELECT:
            pDrvIpv->ipvMode = SiiIpvModeSELECT;
            SiiRegWrite( cInstance,  REG_IPV_MODE, SiiIpvModeSELECT );
            //SiiRegWrite( cInstance,  REG_IP_CONFIG_43, CLEAR_BITS );
            //SiiRegWrite( cInstance,  REG_IP_CONFIG_47, pDrvIpv->pipSpSource );
        	...
        	//SiiRegWrite( cInstance,  REG_PAUTH_RPOVR, (BIT_FORCE_RP_SELECTION | (BIT_FORCE_RP_PORT << pDrvIpv->pipSpSource)) );
        	// Enable Audio Mixing
        	//SiiRegWrite( cInstance,  REG_PAUTH_RP_AOVR, (BIT_FORCE_RP_PORT << pDrvIpv->pipSpSource) );
...
void SiiDrvIpvEnable ( bool_t isEnable )
{
    ...
    if ( isEnable )
    {
        ...
        SiiRegWrite( cInstance,  REG_IPV_WIN_ENABLE, pDrvIpv->enableMask);  // enable window display

        SiiRegWrite( cInstance,  REG_IP_AUTH_PORT_TIME_23_16, 10 );
...
void SiiDrvIpvBorderEnablesSet ( uint8_t borderEnableMask )
{
    ...
    {
        pDrvIpv->lastResultCode = SII_DRV_IPV_SUCCESS;
        SiiRegWrite( cInstance,  REG_IPV_BORDER_CTRL, borderEnableMask & MSK_DB );
    }
}
...
bool_t SiiDrvIpvAnimationIsComplete( BYTE cInstance)
{
    if (pDrvIpv->isAnimationEnabled)
    {
        return SiiRegRead( cInstance,  REG_PV_LOC_CTRL) & BIT_PV_SF_MOVE_START;
    }
    else
    {
        return true;
...
{
    winIndex *= 2;
    SiiRegWrite( cInstance,  REG_IPV_WIN_X0_L + winIndex, (uint8_t)(x >> 0) );
    SiiRegWrite( cInstance,  REG_IPV_WIN_X0_H + winIndex, (uint8_t)(x >> 8) );
    SiiRegWrite( cInstance,  REG_IPV_WIN_Y0_L + winIndex, (uint8_t)(y >> 0) );
    SiiRegWrite( cInstance,  REG_IPV_WIN_Y0_H + winIndex, (uint8_t)(y >> 8) );
...
void SiiDrvIpvWindowAlphaSet ( int_t winIndex, uint8_t newAlpha )
{
    ...
    if ( winIndex == -1 )
    {
        ...
        {
            SiiRegWrite( cInstance,  REG_IPV_ALPHA_0 + i, newAlpha );
        }
    }
    else
    {
        SiiRegWrite( cInstance,  REG_IPV_ALPHA_0 + winIndex, newAlpha );
    }
...
void SiiDrvIpvSizeSet( uint16_t *p16x9, uint16_t *p4x3, uint8_t *pHeight )
{
    ...
    for ( i = 0; i < IPV_WINDOW_COUNT; i++ )
    {
        SiiRegWrite( cInstance,  REG_IPV_1WIN_16x9_WIDTH + i, (uint8_t)(p16x9[i] >> 3 ));
        SiiRegWrite( cInstance,  REG_IPV_1WIN_4x3_WIDTH + i, (uint8_t)(p4x3[i] >> 3 ));

        SiiRegWrite( cInstance,  REG_IPV_1WIN_HEIGHT + i, (uint8_t )(pHeight[i] >> 2 ));
    }
...
bool_t SiiDrvIpvHandleInterrupts( void )
{
    ...
    if ( pDrvIpv->interruptPending )
    {
        ...
        u8Data = SiiRegRead(  cInstance, REG_MP_GCP_STATUS ) & BIT_MP_RES_STABLE;
...
//! @param[in]
//-------------------------------------------------------------------------------------------------
int_t SiiDrvIpvActiveWindowsGet (  BYTE cInstance )
{
    return( SiiRegRead(  cInstance, REG_IPV_WIN_STATUS ) & MSK_IPV_WIN_VALID );
}
//-------------------------------------------------------------------------------------------------
