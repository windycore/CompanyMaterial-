...
#include "uart.h"
#include "uart2.h"
#define MAX_OUTPUT_STR_LEN  120  // 400
...
static bool_t colorEnable = true;
static char outString[MAX_OUTPUT_STR_LEN];     // SkPrintf internal buffer
#ifdef USE_FULL_ASSERT
static bool_t m_bUartPrintEn = TRUE;  //[kyq ADD: 20150824 15:14:48]
#else
static bool_t m_bUartPrintEn = FALSE;
#endif
...
unsigned long ___sprintf ( char *pOutString, const char *pcString, va_list  pArgs ) // void*
{
...
unsigned long SiiSprintf ( char *pOutString, const char *pcString, ... )
{
    ...
    return( ___sprintf( pOutString, pcString,  args )); // kyq  (void *)
}
...
{
    unsigned long   componentNumber, ulCount = 0;
    const char      *pcString;
    va_list         args;
    char           i;
    //[kyq Begin: 20150824 15:12:38]

   if(!m_bUartPrintEn)
     return 0;

//[kyq End: 20150824 15:12:40]
...
//[kyq Begin: 20150916 9:57:30]
   if(0xff !=cInstance)
   {
   
   if(((0x01<<cInstance)& DBG_INSTANCE_EN)== 0)
   	 return 0;
    ulCount += SiiSprintf( &outString[ulCount], "{%d:}", cInstance );	    

   }
//[kyq End: 20150916 9:57:32]

    va_start(args, printFlags);
    if ( printFlags & DBGF_CN ) // 打印模块名字
    {
        componentNumber = va_arg( args, unsigned long);
	 if(((0x01 << componentNumber)&DBG_COMPONENTS_EN)==0)
	 	return 0;
...
void SiiPlatformLoggerMessageLevelSet ( uint_t msgLevel )
{
    ...
    if(platform.dbgMsgLevel >0)
	m_bUartPrintEn = TRUE;
    else
	m_bUartPrintEn = FALSE;
}
...
{
    DEBUG_PRINT2(0,"Wrong parameters value: file %s on line %d\r\n", pcFilename, ulLine);
}
