...
//          |                                        |
//在串口数据向APP 数据BUF 拷贝时候
//     - WaitStartRX:  开始判断数据包头, 收到包头(0xff, 's')   --> ReadyRX
//     - ReadyRX :     把串口数据向app buf 拷贝
//
//  	app : 一个完整的包收到后设为WaitStartRX
//
...
//SkSerialRxHandler 会根据收到的第一个数据设置
// 应该使用SOURCE_IS_SIMON
   ...
   if (!SkSerialConsumedChar()) // isBuffInUse == false
    {
           ...
           SkSerialRxHandler(cChar);  // 复制数据到 APP buf
     }
