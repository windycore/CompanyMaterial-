...
// 发送数据给远端
    case pwrON:
       // if (m_ePowerState != pwrSTANDBY)
        ...
        for (i = 0; i < DevNum; i++)
        {
            dvSii9533PowerControl(i, APP_POWERSTATUS_STANDBY_TO_ON);
        }
        m_ePowerState = pwrON;
    ...
    case pwrSTANDBY:
        ...
        for (i = 0; i < DevNum; i++)
        {
            dvSii9533PowerControl(i, APP_POWERSTATUS_ON_TO_STANDBY);  // will let AD4604 i2c error, not know why
        }
...
static void PowerMgrUpdateLocalTimer(void)
{
    ...
    if (abs(m_wPowerTimer - m_wLastSecondsTimer) >= TICKS_PER_SECOND)
    {
        m_wSecondsTimer += 1;
...
// Avoid defining local variable here.   (Especically, no assignment command follows!!! )
    ...
    case pwrSOFTOFF:
        ...
        if (abs(m_wPowerTimer - m_wTransTimer )> SOFTOFF_DELAY)
        {
            ...
            SiiPlatformTimerSet(etLINK_SCAN,mS_TO_TICKS((5000L)*2),0xFF); // use systick: 1ms per tick
            #endif
    ...
    case pwrSOFTRESET:
        ...
        if (abs(m_wPowerTimer - m_wTransTimer) > SOFTRESET_DELAY)
        {
            appUIEnable(FALSE); //[kyq ADD: 20150907 20:40:51]
            utilDataMgrUpdate(UPDATE_ALL_NODEID);
            m_eNextPowerState = pwrON; //[kyq add: 20150819 0:45:47]
...
// For now we are not doing anything special just set the state to on.
    case pwrON:
        if (abs(m_wPowerTimer - m_wTransTimer) > POWER_ON_DELAY)
        {
            //m_ePowerState = m_eNextPowerState; // 不改变状态，在PowerMgrStartStateTransition 后改变
            halPowerCtrlPowerSet(pwrON);
	     PowerMgrStartStateTransition(pwrON);
