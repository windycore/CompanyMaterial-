...
void flash_set_status(sii_inst_t inst, enum flash_status status)
{
...
enum sii_status flash_init(sii_inst_t inst)
{
 ...
 enum sii_status flash_erase(sii_inst_t inst)
{
	enum sii_status ret = SII_STATUS_SUCCESS;
...
erase_exit:
 ...
 enum sii_status flash_write_block(sii_inst_t inst, uint32_t address,
...
write_exit:
 ...
 enum sii_status flash_read_block(sii_inst_t inst, uint32_t address,
	uint16_t len, uint8_t *block)
{
	enum sii_status ret = SII_STATUS_SUCCESS;
	uint8_t addrees_buffer[4];
