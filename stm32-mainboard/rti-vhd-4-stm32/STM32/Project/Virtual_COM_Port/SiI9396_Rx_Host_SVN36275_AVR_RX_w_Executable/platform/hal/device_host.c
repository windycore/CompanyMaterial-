...
{
	#if(MACHINETYPE==MUH44A_H2)			// 0-3为分别控制4个输入的sil9396，4-7为分别控制4个输出的sil9396
	eI2CBus9396_IN,
	eI2CBus9396_IN,
	eI2CBus9396_IN,
	eI2CBus9396_IN,
	...
	eI2CBus9396_OUT,
	eI2CBus9396_OUT
	#else									// 0-7为分别控制8个输入的sil9396，8-15为分别控制8个输出的sil9396
	eI2CBus9396_IN,
	eI2CBus9396_IN,
	eI2CBus9396_IN,
	eI2CBus9396_IN,
...
void device_slect_9396(BYTE deviceID)
{
    #if(MACHINETYPE==MUH44A_H2)		//范围是0到7
    switch(deviceID)
    {
	    case 0:
		 dvSelectChilpEnable(eSil9396IN_EN);
	    ...
	    case 3:
		 dvSelectChilpEnable(eSil9396IN_EN);
	        ...
	        break;
	    case 4:
		 dvSelectChilpEnable(eSil9396OUT_EN);
	        GPIOB->BRR = GPIO_Pin_5 ;
	        GPIOB->BRR = GPIO_Pin_4 ;
	        break;
	    case 5:
    ...
    switch(deviceID)
    {
	    ...
	    case 3:
		 dvSelectChilpEnable(eSil9396IN_EN);
	        ...
	        break;
	    case 4:
		 dvSelectChilpEnable(eSil9396IN_EN2);
	        GPIOB->BRR = GPIO_Pin_5 ;
	        GPIOB->BRR = GPIO_Pin_4 ;
	        break;
	    case 5:
	    ...
	    case 11:
		 dvSelectChilpEnable(eSil9396OUT_EN);
	        ...
	        break;
	    case 12:
		 dvSelectChilpEnable(eSil9396OUT_EN2);
	        GPIOB->BRR = GPIO_Pin_5 ;
	        GPIOB->BRR = GPIO_Pin_4 ;
	        break;
	    case 13:
...
free_tx_bytes:
	...
	device_slect_9396(inst);
    	eResult = dvDeviceBusRead(  m_acI2CBus[inst],sii_dev[inst].slave_addr, realaddr, len,data_block, FLG_WADDR);    
    	retVal = (eResult==rcSUCCESS)?SII_STATUS_SUCCESS:SII_STATUS_ERR_FAILED;

	#if 0
	retVal = sii_hal_op(hwid, i2c_tx_bytes, 2, data_block, len);
	if (SII_STATUS_SUCCESS != retVal) {
		err("I2C read block failed!");
		goto exit;
	}
	#endif
...
int sii_device_reg_write_block(
	...
	/* Allocate the Transfer Buffer */
	tx_bytes = (uint8_t *)
		sii_os_calloc("I2C TX Buffer", (len + 2), 0);
	if (NULL == tx_bytes) {
		err("Out of memory\n");
		retVal = SII_STATUS_ERR_FAILED;
		goto exit;
	...
	tx_bytes[0] = page_num;
    	...
    	realaddr = (page_num<<8)|offset;
	device_slect_9396(inst);
    	eResult = dvDeviceBusWrite( m_acI2CBus[inst],sii_dev[inst].slave_addr, realaddr,len, &(tx_bytes[2]), FLG_WADDR );
    	retVal = (eResult==rcSUCCESS)?SII_STATUS_SUCCESS:SII_STATUS_ERR_FAILED;

	#if 0
	retVal = sii_hal_op(hwid, tx_bytes, len + 2, NULL, 0);
	if (SII_STATUS_SUCCESS != retVal) {
		err("I2C write block failed!");
		goto free_tx_bytes;
	}
	#endif
#endif
